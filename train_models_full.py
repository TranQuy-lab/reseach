...
LOG_FILE = "/workspace/reseach/output/train_full.log"
os.makedirs("/workspace/reseach/output", exist_ok=True)
...
def main():
    ...
    train_path = "/workspace/reseach/data/dl_processed/train"
    val_path   = "/workspace/reseach/data/dl_processed/val"
    ...
    torch.save(model_anomale.state_dict(), "/workspace/reseach/output/anomale_best.pt")
    ...
    with open("/workspace/reseach/output/results_full_13m.json", "w") as f:
        json.dump(results, f, indent=2)

    logger.info("=" * 75)
    logger.info("=== TOÀN BỘ QUÁ TRÌNH HUẤN LUYỆN ĐÃ HOÀN TẤT THÀNH CÔNG ===")
    logger.info(f"Kết quả lưu tại: /workspace/reseach/output/results_full_13m.json")
    logger.info("=" * 75)
